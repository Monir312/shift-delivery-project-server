...
const verifyFBToken = async (req, res, next) => {
  ...
  if (!idToken) return res.status(401).send({
...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.lrsmj0e.mongodb.net/?appName=Cluster0`;
...
async function run() {
  try {
    ...
    app.get('/users', verifyFBToken, async (req, res) => {
      ...
      const cursor = userCollection.find(query).sort({
        createdAt: -1
      }).limit(5);
    ...
    app.post('/payment-checkout-session', async (req, res) => {
      try {
        ...
        const amount = parseInt(paymentInfo.cost) * 100; // Convert USD to cents
    ...
    app.patch('/riders/:id', async (req, res) => {
      ...
      if (status === 'approved') {
        ...
        userResult = await userCollection.updateOne(userQuery, updatedUser);
      }
